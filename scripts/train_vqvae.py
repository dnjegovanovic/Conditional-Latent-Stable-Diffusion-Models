...
from typing import Any, Dict  # Type annotations for clarity
...
from con_stable_diff_model.utils.dataset_factory import build_dataset  # Shared dataset builder
from con_stable_diff_model.models.Discriminator import Discriminator  # PatchGAN discriminator
from con_stable_diff_model.models.LPIPS import LPIPS  # Perceptual loss model
from con_stable_diff_model.modules.VQVAE import VectorQuantizedVAE as VQVAE  # VQ-VAE model
...
def load_config(
    path: Path,
) -> Dict[str, Any]:  # Load YAML configuration into a dictionary
    ...
    with path.open("r", encoding="utf-8") as handle:  # Open the YAML file safely
        try:  # Guard against malformed YAML
            ...
            ) from error  # Re-raise with context
...
def set_seed(seed: int) -> None:  # Ensure reproducibility across libraries
    ...
    torch.backends.cudnn.deterministic = (
        True  # Encourage deterministic convolutions where possible
    )
...
def save_image_grid(
    inputs: Tensor, recons: Tensor, out_dir: Path, step: int
) -> None:  # Save input/recon grids
    ...
    combined = torch.cat(
        [inputs, recons], dim=0
    )  # Stack original and reconstructed batches
    grid = vutils.make_grid(
        combined, nrow=inputs.size(0)
    )  # Arrange images into a single grid tensor
    output_path = (
        out_dir / f"current_autoencoder_sample_{step}.png"
    )  # Build deterministic filename
    vutils.save_image(grid, output_path)  # Write the grid to disk


def accumulate(
    loss: Tensor, factor: int
) -> Tensor:  # Utility for gradient accumulation scaling
...
def train(config: Dict[str, Any]) -> None:  # Main training routine
    ...
    model = VQVAE(
        input_channels=dataset_cfg["im_channels"], VQVAE=autoencoder_cfg
    ...
    )  # Instantiate PatchGAN discriminator
    ...
    output_dir = Path(
        train_cfg["task_name"]
    )  # Directory where checkpoints/visuals are saved
    ...
    optimizer_g = Adam(
        model.parameters(), lr=train_cfg["autoencoder_lr"], betas=(0.5, 0.999)
    )  # Generator optimizer
    optimizer_d = Adam(
        discriminator.parameters(), lr=train_cfg["autoencoder_lr"], betas=(0.5, 0.999)
    )  # Discriminator optimizer

    accumulation_steps = max(
        1, train_cfg.get("autoencoder_acc_steps", 1)
    )  # Gradient accumulation factor
    save_every = max(
        1, train_cfg.get("autoencoder_img_save_steps", 100)
    )  # Frequency for saving images
    disc_start = train_cfg.get(
        "disc_start", 0
    )  # Delay before adversarial training kicks in

    codebook_weight = train_cfg.get(
        "codebook_weight", 1.0
    )  # Weighting for codebook loss
    commitment_beta = train_cfg.get(
        "commitment_beta", 0.25
    )  # Weighting for commitment loss
    perceptual_weight = train_cfg.get(
    ...
    for epoch in range(total_epochs):  # Epoch-wise training loop
        epoch_recon, epoch_codebook, epoch_perceptual = (
            [],
            [],
            [],
        )  # Accumulate generator metrics
        epoch_adv_fake, epoch_disc = (
            [],
            [],
        )  # Accumulate adversarial generator/discriminator losses

        optimizer_g.zero_grad(
            set_to_none=True
        )  # Reset generator gradients before epoch
        optimizer_d.zero_grad(
            set_to_none=True
        )  # Reset discriminator gradients before epoch

        for batch in tqdm(
            data_loader, desc=f"Epoch {epoch + 1}/{total_epochs}"
            ...
            recon, quantized, quant_losses = model(
                images
            )  # Forward pass through VQ-VAE (returns recon + losses)

            if (
                global_step == 1 or global_step % save_every == 0
                ...
                with torch.no_grad():  # Disable grad computations for logging
                    ...
                    inputs_vis = (
                        images[:sample_count].detach().cpu() + 1
                    ) / 2  # Rescale inputs to [0,1]
                    recons_vis = (
                        recon[:sample_count].detach().cpu().clamp(-1, 1) + 1
                    ) / 2  # Clamp and rescale reconstructions
                    save_image_grid(
                        inputs_vis, recons_vis, sample_dir, sample_index
                    )  # Persist visualization grid
                    sample_index += 1  # Increment saved grid counter

            recon_loss = recon_loss_fn(recon, images)  # Compute reconstruction MSE
            codebook_loss = (
                codebook_weight * quant_losses["codebook"]
            )  # Weighted codebook loss
            commitment_loss = (
                commitment_beta * quant_losses["commitment"]
            )  # Weighted commitment loss

            gen_loss = accumulate(
                recon_loss, accumulation_steps
            )  # Start generator loss with scaled reconstruction term
            gen_loss = gen_loss + accumulate(
                codebook_loss, accumulation_steps
            )  # Add codebook component
            gen_loss = gen_loss + accumulate(
                commitment_loss, accumulation_steps
            )  # Add commitment component

            epoch_recon.append(recon_loss.item())  # Track raw reconstruction loss
            epoch_codebook.append(
                codebook_loss.item()
            )  # Track raw codebook contribution

            with torch.no_grad():  # Freeze LPIPS model gradients
                lpips_value = lpips_model(
                    recon, images
                ).mean()  # Compute mean perceptual distance
            perceptual_term = perceptual_weight * lpips_value  # Apply weighting
            gen_loss = gen_loss + accumulate(
                perceptual_term, accumulation_steps
            )  # Add perceptual loss to generator objective
            epoch_perceptual.append(
                perceptual_term.item()
            )  # Log perceptual contribution

            if (
                global_step > disc_start
            ):  # Adversarial loss only after specified warm-up
                fake_pred = discriminator(
                    recon
                )  # Predict realism of reconstructed images
                adv_target = torch.ones_like(
                    fake_pred, device=DEVICE
                )  # Target label (real) for generator
                adv_loss = adv_loss_fn(
                    fake_pred, adv_target
                )  # Least-squares adversarial loss
                epoch_adv_fake.append(
                    (disc_weight * adv_loss).item()
                )  # Log weighted adversarial contribution
                gen_loss = gen_loss + accumulate(
                    disc_weight * adv_loss, accumulation_steps
                )  # Include adversarial term

            gen_loss.backward()  # Backprop generator objective

            if (
                global_step % accumulation_steps == 0
            ):  # Apply optimizer step when accumulation window completes
            ...
            if global_step > disc_start:  # Update discriminator only after warm-up
                ...
                fake_pred_detached = discriminator(
                    recon.detach()
                )  # Prediction for detached reconstructions

                real_labels = torch.ones_like(
                    real_pred, device=DEVICE
                )  # Real target tensor
                fake_labels = torch.zeros_like(
                    fake_pred_detached, device=DEVICE
                )  # Fake target tensor

                real_loss = adv_loss_fn(
                    real_pred, real_labels
                )  # Discriminator loss on real samples
                fake_loss = adv_loss_fn(
                    fake_pred_detached, fake_labels
                )  # Discriminator loss on fake samples

                disc_loss = (
                    disc_weight * 0.5 * (real_loss + fake_loss)
                )  # Combine losses with weighting and average
                epoch_disc.append(disc_loss.item())  # Log discriminator loss

                accumulate(
                    disc_loss, accumulation_steps
                ).backward()  # Backprop scaled discriminator loss

                if (
                    global_step % accumulation_steps == 0
                ):  # Apply discriminator update on accumulation boundary
        ...
        if (
            global_step % accumulation_steps != 0
        ):  # Flush pending generator gradients at epoch end
        ...
        if (
            global_step % accumulation_steps != 0 and epoch_disc
        ):  # Flush discriminator grads if needed and applicable
            optimizer_d.step()  # Update discriminator parameters
            optimizer_d.zero_grad(set_to_none=True)  # Reset gradients

        recon_mean = (
            float(np.mean(epoch_recon)) if epoch_recon else 0.0
        )  # Compute epoch-level reconstruction loss
        perceptual_mean = (
            float(np.mean(epoch_perceptual)) if epoch_perceptual else 0.0
        )  # Epoch perceptual loss
        codebook_mean = (
            float(np.mean(epoch_codebook)) if epoch_codebook else 0.0
        )  # Epoch codebook loss
        adv_mean = (
            float(np.mean(epoch_adv_fake)) if epoch_adv_fake else 0.0
        )  # Epoch adversarial generator loss
        disc_mean = (
            float(np.mean(epoch_disc)) if epoch_disc else 0.0
        ...
        torch.save(
            model.state_dict(), output_dir / train_cfg["vqvae_autoencoder_ckpt_name"]
        )  # Persist VQ-VAE weights
        torch.save(
            discriminator.state_dict(),
            output_dir / train_cfg["vqvae_discriminator_ckpt_name"],
        )  # Persist discriminator weights
...
def parse_args() -> argparse.Namespace:  # Command-line interface builder
    ...
    parser.add_argument(
        "--config", dest="config_path", default="./ulsd_model/config.yml", type=str
    )  # Add config flag
