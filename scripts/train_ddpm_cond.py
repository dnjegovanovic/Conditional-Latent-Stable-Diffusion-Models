...
from typing import (
    ...
    Optional,
    Tuple,
)  # Expose common typing aliases used throughout the script
...
from torch import (
    Tensor,
)  # Clarify Tensor type for annotations without qualifying torch.Tensor
...
from con_stable_diff_model.models.LinearNoiseScheduler import (
    LinearNoiseScheduler,
)  # Diffusion noise scheduler implementation
from con_stable_diff_model.modules.VQVAE import (
    VectorQuantizedVAE as VQVAE,
)  # Pretrained VQ-VAE for encoding/decoding images
from con_stable_diff_model.modules.UNet import (
    UNet,
)  # Conditional UNet backbone used as denoiser
...
def load_config(
    ...
    if not path.exists():  # Guard against missing configuration files
        ...
        )  # Emit helpful error message when path is invalid
    with path.open(
        "r", encoding="utf-8"
    ) as handle:  # Open config file with explicit UTF-8 encoding
        return yaml.safe_load(
            handle
        )  # Parse YAML contents into native Python structures
...
def set_seed(seed: int) -> None:  # Make training deterministic given a provided seed
    ...
    if DEVICE.type == "cuda":  # Apply CUDA-specific seeding when GPU is active
        torch.cuda.manual_seed_all(
            seed
        )  # Seed all CUDA devices for consistent behaviour
    torch.backends.cudnn.deterministic = True  # Force deterministic CuDNN operations
    torch.backends.cudnn.benchmark = (
        False  # Disable CuDNN auto-tuner to avoid nondeterminism
    )
...
def ensure_dir(path: Path) -> None:  # Create directories if they are missing
    ...
    )  # Recursively create directories, ignoring if already present


def save_image_grid(
    images: Tensor, directory: Path, step: int, nrow: int, prefix: str
) -> None:  # Persist qualitative image grids
    ...
    grid = vutils.make_grid(  # Assemble a grid of images for viewing
        ...
        nrow=max(
            1, min(nrow, images.size(0))
        ),  # Cap row count so it never exceeds batch size
        padding=2,  # Add spacing between images in the grid
    )
    vutils.save_image(
        grid, directory / f"{prefix}_{step:06d}.png"
    )  # Write the grid to disk with step-indexed filename


def decode_latents_to_images(
    latents: Tensor, vqvae: VQVAE
) -> Tensor:  # Convert latent tensors back into image space
...
def sample_diffusion_latents(  # Perform reverse diffusion sampling to generate latents
    ...
    cond_input: Optional[
        Dict[str, Tensor]
    ] = None,  # Optional conditioning dictionary for guided sampling
) -> Tensor:
    ...
    with torch.no_grad():  # Disable autograd during sampling
        latents = torch.randn(
            (num_samples, *latent_shape), device=DEVICE
        )  # Initialize latents with Gaussian noise
        predicted_x0: Optional[Tensor] = (
            None  # Track the most recent reconstructed clean sample
        )
        for timestep in range(
            scheduler.num_timesteps - 1, -1, -1
        ):  # Iterate timesteps backwards through diffusion process
            t_tensor = torch.full(
                (num_samples,), timestep, device=DEVICE, dtype=torch.long
            )  # Create tensor filled with current timestep index
            noise_pred = model(
                latents, t_tensor, cond_input=cond_input
            )  # Predict noise using the UNet conditioned on timestep (and optional class)
            latents, predicted_x0 = scheduler.sample_prev_timestep(
                latents, noise_pred, timestep
            )  # Step to previous timestep using scheduler equations
        samples = (
            predicted_x0 if predicted_x0 is not None else latents
        )  # Prefer final predicted clean samples when available
    ...
    return (
        samples.detach()
    )  # Return detached samples to avoid gradient tracking downstream
...
def prepare_class_condition(  # Convert integer labels into one-hot conditioning with optional dropout
    ...
    condition_config: Dict[
        str, Any
    ],  # Configuration chunk describing class conditioning behaviour
) -> Tensor:
    """Convert integer class targets to (optionally dropped) one-hot embeddings."""  # Clarify transformation performed
    class_cfg = condition_config[
        "class_condition_config"
    ]  # Extract class conditioning parameters
    num_classes = class_cfg["num_classes"]  # Determine total number of classes
    drop_prob = float(
        get_config_value(class_cfg, "cond_drop_prob", 0.0)
    )  # Pull optional dropout probability for classifier-free guidance

    class_indices = class_indices.to(
        DEVICE
    )  # Move incoming class indices onto active device
    class_tensor = F.one_hot(
        class_indices, num_classes=num_classes
    ).float()  # Create one-hot encodings for each index

    if drop_prob > 0.0:  # Apply dropout when probability is non-zero
        drop_mask = (
            torch.rand(class_tensor.size(0), device=class_tensor.device) < drop_prob
        ).unsqueeze(
            -1
        )  # Sample Bernoulli mask per example
        class_tensor = torch.where(
            drop_mask, torch.zeros_like(class_tensor), class_tensor
        )  # Replace dropped embeddings with zeros
...
def train(
    config: Dict[str, Any]
) -> None:  # Execute the DDPM training loop using supplied configuration
    """Train a conditional DDPM on VQ-VAE latents."""  # Provide overview of training routine
    diffusion_cfg = config[
        "diffusion_params"
    ]  # Extract diffusion hyperparameters controlling noise schedule
    dataset_cfg = config[
        "dataset_params"
    ]  # Retrieve dataset configuration for data loading
    ...
    diffusion_model_cfg = config.get(
        "cond_model", {}
    )  # Optionally grab conditional model configuration block

    condition_config = get_config_value(
        diffusion_model_cfg, "condition_config", None
    )  # Extract conditioning options if present
    condition_types = (
        condition_config["condition_types"] if condition_config else []
    ...
    if "class" in condition_types:  # Validate class conditioning when requested
        validate_class_config(
            condition_config
        )  # Ensure required keys like num_classes exist
    if (
        "image" in condition_types
    ):  # Validate image conditioning configuration when requested
        validate_image_config(
            condition_config
    ...
    )  # Instantiate dataset according to configuration
    data_loader = DataLoader(  # Wrap dataset with DataLoader for batching
        ...
        num_workers=train_cfg.get(
            "num_workers", 0
        ),  # Optional background workers for data loading
        pin_memory=DEVICE.type == "cuda",  # Pin host memory to speed up GPU transfers
    )

    output_dir = Path(
        train_cfg.get("task_name_ddpm_cond", train_cfg["task_name"])
    )  # Directory to store checkpoints and samples
    vqvae_dir = Path(
        train_cfg["vqvae_model"]
    )  # Directory holding VQ-VAE checkpoints and latents
    ...
    scheduler = LinearNoiseScheduler(  # Instantiate diffusion noise scheduler using config parameters
        ...
        num_timesteps=diffusion_cfg[
            "num_timesteps"
        ],  # Total diffusion steps to traverse
        beta_start=diffusion_cfg[
            "beta_start"
    ...
    unet_params = dict(
        unet_cfg
    )  # Clone UNet config to avoid mutating original dictionary
    unet_params["im_channels"] = vqvae_cfg[
        "z_channels"
    ]  # Override input channels to match encoded latent channels
    if diffusion_model_cfg:  # Attach conditional model configuration when available
        unet_params["model_config"] = (
            diffusion_model_cfg  # Pass conditioning configuration into UNet constructor
        )
    ...
    vqvae = VQVAE(  # Instantiate VQ-VAE used to convert images to latent space
        input_channels=dataset_cfg[
            "im_channels"
        ],  # Match dataset image channel count (e.g., 1 for MNIST)
    ...
    for param in vqvae.parameters():  # Iterate over VQ-VAE parameters
        param.requires_grad = (
            False  # Disable gradients to keep VQ-VAE frozen during diffusion training
        )

    vqvae_ckpt = (
        vqvae_dir / train_cfg["vqvae_autoencoder_ckpt_name"]
    )  # Construct path to VQ-VAE checkpoint
    ...
    )  # Load pretrained VQ-VAE weights onto device

    optimizer = Adam(
        model.parameters(), lr=train_cfg["ldm_lr"]
    )  # Configure Adam optimizer using learning rate from config
    criterion = (
        torch.nn.MSELoss()
    )  # Choose mean squared error loss for noise prediction objective
    num_epochs = train_cfg["ldm_epochs"]  # Determine number of epochs to train

    samples_root = (
        output_dir / "ddpm_samples"
    )  # Base directory for storing generated sample grids
    ...
    )  # Determine interval for saving sample grids
    viz_samples = max(
        1, int(train_cfg.get("num_samples", 16))
    )  # Number of samples to visualize per save event
    viz_rows = max(
        1, int(train_cfg.get("num_grid_rows", int(np.ceil(np.sqrt(viz_samples)))))
    )  # Row count for image grid layout
    ...
    latent_shape: Optional[Tuple[int, ...]] = (
        None  # Record latent tensor shape once determined
    )
    ...
    for epoch in range(num_epochs):  # Iterate through each training epoch
        ...
        for batch in tqdm(
        ...
        ):  # Loop over batches with progress feedback
            ...
            ).float()  # Move images to device and ensure float dtype

            optimizer.zero_grad(
                set_to_none=True
            )  # Reset gradients efficiently before forward pass

            with torch.no_grad():  # Freeze gradients while encoding with VQ-VAE
                latents, _ = vqvae.encode(
                    images
                )  # Encode images into latent space (discarding quantization losses)
            latents = latents.detach()  # Detach latents from computation graph

            if (
                latent_shape is None
            ):  # Record latent tensor shape once for sampling routine
                latent_shape = tuple(
                    latents.shape[1:]
                )  # Capture channel and spatial dimensions

            cond_input: Optional[Dict[str, Tensor]] = (
                None  # Initialize conditioning dictionary passed to UNet
            )
            if condition_config and raw_cond:  # Only process conditioning when provided
                ...
                if (
                    "class" in condition_types
                ):  # Convert class labels into embeddings when enabled
                    class_indices = raw_cond["class"].to(
                        DEVICE
                    )  # Move label indices to current device
                    cond_input["class"] = prepare_class_condition(
                        class_indices, condition_config
                    )  # Populate class conditioning entry
                if (
                    "image" in condition_types
                ):  # Integrate auxiliary image/mask conditioning when requested
                    assert (
                        "image" in raw_cond
                    ), "Conditioning Type Image but dataset did not provide image conditioning input"
                    image_condition = (
                        raw_cond["image"].to(DEVICE).float()
                    )  # Move masks to device for UNet use
                    im_drop_prob = float(
                        get_config_value(
            ...
            )  # Corrupt clean latents according to sampled timesteps

            noise_pred = model(
                noisy_latents, timesteps, cond_input=cond_input
            )  # Predict noise using conditional UNet
            loss = criterion(
                noise_pred, noise
            )  # Compute denoising objective as MSE between predicted and target noise
            ...
            if (
                global_step % save_every == 0 and latent_shape is not None
            ):  # Periodically save reconstructions and samples
                real_count = min(
                    viz_samples, latents.size(0)
                )  # Determine number of real examples to visualize
                real_latents = latents[
                    :real_count
                ].detach()  # Slice latent batch for visualization
                real_images = decode_latents_to_images(
                    real_latents, vqvae
                ...
                if condition_config and latent_shape is not None:
                    sample_cond_input = {}
                    if (
                        "class" in condition_types
                    ):  # Sample random classes for guidance snapshots
                        num_classes = int(
                        ...
                        class_one_hot = F.one_hot(
                            sample_classes, num_classes=num_classes
                        ).float()  # Convert to one-hot embeddings
                        sample_cond_input["class"] = (
                            class_one_hot  # Supply conditioning dictionary expected by the UNet
                        )
                    if "image" in condition_types and raw_cond and "image" in raw_cond:
                ...
                sampled_latents = (
                    sample_diffusion_latents(  # Generate samples via reverse diffusion
                        model=model,
                        scheduler=scheduler,
                ...
                sampled_images = decode_latents_to_images(
                    sampled_latents, vqvae
                )  # Decode generated latents to image space

                save_image_grid(
                    real_images, recon_dir, sample_index, viz_rows, "recon"
                )  # Save reconstruction grid to disk
                save_image_grid(
        ...
        )  # Compute mean loss over the epoch
        print(
            f"Epoch {epoch + 1}/{num_epochs} | Loss {mean_loss:.4f}"
        )  # Report epoch progress and loss
...
def parse_args() -> argparse.Namespace:  # Parse command-line options for this script
    parser = argparse.ArgumentParser(
        description="Arguments for conditional DDPM training"
    )  # Describe script purpose in CLI help
...
if (
    __name__ == "__main__"
):  # Ensure script runs only when executed directly, not imported
