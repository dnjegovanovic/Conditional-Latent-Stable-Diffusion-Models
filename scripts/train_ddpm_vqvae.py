...
from typing import Any, Dict, Optional, Tuple  # Expose common typing primitives
...
from con_stable_diff_model.modules.UNet import UNet  # Denoising network architecture

from con_stable_diff_model.utils.dataset_factory import build_dataset
...
def train(config: Dict[str, Any]) -> None:  # Main entry point for DDPM training
    ...
    dataset = build_dataset(  # Construct dataset (latents preferred when available)
        ...
        use_latents_override=use_cached_latents,
        latent_path_override=l_path if use_cached_latents else None,
    )
...
def parse_args() -> argparse.Namespace:  # Build CLI argument parser
    ...
    parser.add_argument(  # Define configuration file flag
        "--config",
        dest="config_path",
        default="./con_stable_diff_model/config_vqvae_celeb.yml",
        type=str,
    )
