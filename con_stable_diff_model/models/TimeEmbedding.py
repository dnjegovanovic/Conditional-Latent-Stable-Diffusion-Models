...
def time_embedding_fun(
    ...
    time_embedding_dim: int = 320,  # torch.tensor = torch.tensor([320])
) -> torch.Tensor:
    """
    Computes time embedding using sinusoidal positional encoding.

    Args:
        timestep: Input timestep tensor of shape (B,) (batch of timesteps)
        time_embedding_dim: Embedding dimension (must be even number)

    Returns:
        Time embedding tensor of shape (B, 2*time_embedding_dim)
    ...
    # timestep shape: (B,) -> (B, 1) -> (B, d_model//2) via repetition
    t_emb = timestep[:, None].repeat(1, time_embedding_dim // 2) / factor
